...
func (wkf *Workflow) Execute(ctx context.Context, config *viperx.CViper, fr *FunctionRegistry, logger logx.Logger) error {
	...
	// Get tiers: each tier is a set of phases
	tiers, err := wkf.TopoTierSorted()
	if err != nil {
		return fmt.Errorf("sorting tiers: %w", err)
	}
	...
	for i, tier := range tiers {
		...
		// Channel to collect errors from all phases in this tier
		...
		for _, phase := range tier {
			...
			go func(p Phase) {
				...
				// Execute the phase
				if _, err := p.Execute(ctx, config, fr, logger); err != nil {
					errCh <- fmt.Errorf("phase %s failed: %w", p.Name, err)
				}
			}(phase)
		}

		// Wait for all phases in this tier to complete
		wg.Wait()
		close(errCh)

		// Aggregate tier errors
		var tierErrs []error
		for e := range errCh {
