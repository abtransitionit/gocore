...
// Description: get the image id for a given vps

// Description: get the list of images available for a VPS
//
// Notes:
//   - each VPS has its own set of images.
func ImageAvailableGetList(ctx context.Context, vpsNameOrId string, logger logx.Logger) ([]ImageDetail, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("building path for %s with id %s >%w", ep.Desc, vpsId, err)
	}
	...
	for _, imdItem := range respData {
		...
		go func(onItem string) {
			defer wg.Done()
			// logger.Debugf("getting detail for img %s", onItem)
...
func imageGetDetail(ctx context.Context, vpsNameOrId string, imgId string, logger logx.Logger) (*ImageDetail, error) {
	...
	// 6 - Play the request and get response
	// logger.Debugf("%s using endpoint %s", ep.Desc, endpoint)
	err = client.Do(ctx, req, &respData)
	if err != nil {
		return nil, fmt.Errorf("API request failed to %s : %w", ep.Desc, err)
	}
...
func VpsGetImage(ctx context.Context, vpsNameOrId string, logger logx.Logger) ([]ImageDetail, error) {
	// 1 - define response structure
	var respData []string
	...
	// Get VPS Distro from local config
